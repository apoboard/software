...
float rgain = 0.9; //multiplier for red LED channel intensity (typically ranges 0 - 2)
float ggain = 0.5; //multiplier for green LED channel intensity (typically ranges 0 - 2)
float bgain = 1; //multiplier for blue LED channel intensity (typically ranges 0 - 2)

int fftstartingbin = 8;
...
int buttonstate = 0;
int mode = 0;
int numberofmodes = 8; //total number of modes (controls the wrap-around of toggling modes)
...
//      Serial.print("--");
      ...
      largestbinsmoothmapped = largestbinsmooth / ((FHT_N/4) - fftstartingbin);
      if (largestbinsmoothmapped > 1) {
        largestbinsmoothmapped = 1;
      }
      ...
      ledhue = ((-1*(-largestbinsmoothmapped))+(-2*totalbinpower))+hoffset;
      ledvalue = 1*totalbinpower;
